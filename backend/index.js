...
app.post("/update-run/:matchCode", async (req, res) => {
  try {
    ...
    delete matchClone._id;
    delete matchClone.__v;
    
    match.lastAction = {
      matchSnapshot: matchClone,
      timestamp: new Date(),
      actionType: "run",
    };
...
app.post("/undo-last-action", async (req, res) => {
  try {
    const { matchCode } = req.body;
    const match = await CurrentMatch.findOne({ matchCode });
    if (!match || !match.lastAction)
      return res.status(404).json({ success: false, message: "Undo not possible" });

    const snapshot = match.lastAction.matchSnapshot;
    if (!snapshot)
      return res.status(400).json({ success: false, message: "No snapshot found" });

    // Identify active players before undo
    const currentBatter1 = match.activeBatter1;
    const currentBatter2 = match.activeBatter2;
    const currentBowlerId = match.activeBowler;

    const snapshotBatter1 = snapshot.activeBatter1;
    const snapshotBatter2 = snapshot.activeBatter2;
    const snapshotBowlerId = snapshot.activeBowler;

    // Load user documents
    const batter1User = await User.findById(snapshotBatter1.player);
    const batter2User = await User.findById(snapshotBatter2.player);
    const bowlerUser = await User.findById(snapshotBowlerId);

    const playersBefore = match.playersA.concat(match.playersB);
    const playersSnapshot = snapshot.playersA.concat(snapshot.playersB);

    const subtractStats = (user, playerId, type) => {
      const current = playersBefore.find(p => p.id?.toString() === playerId?.toString());
      const old = playersSnapshot.find(p => p.id?.toString() === playerId?.toString());

      if (user && current && old && current.stats?.[type] && old.stats?.[type]) {
        const keys = Object.keys(old.stats[type]);
        for (const key of keys) {
          if (typeof current.stats[type][key] === 'number' && typeof old.stats[type][key] === 'number') {
            const delta = current.stats[type][key] - old.stats[type][key];
            user.stats[type][key] -= delta;
            if (user.stats[type][key] < 0) user.stats[type][key] = 0; // safety
          }
        }
      }
    };

    if (batter1User) subtractStats(batter1User, snapshotBatter1.player, "batting");
    if (batter2User) subtractStats(batter2User, snapshotBatter2.player, "batting");
    if (bowlerUser) subtractStats(bowlerUser, snapshotBowlerId, "bowling");

    // Restore match snapshot (but only for match state, not users)
    Object.assign(match, snapshot); // don't use .overwrite here
    match.lastAction = null;

    // Save everything
    await match.save();
    if (batter1User) await batter1User.save();
    if (batter2User) await batter2User.save();
    if (bowlerUser) await bowlerUser.save();

    res.json({ success: true, message: "Undo successful", match });
  } catch (error) {
    console.error("Undo error:", error);
    res.status(500).json({ success: false, message: "Undo failed", error: error.message });
  }
});
...
// Deep copy match document (remove internal fields)
const matchSnapshot = JSON.parse(JSON.stringify(match));
delete matchSnapshot._id;
delete matchSnapshot.__v;
